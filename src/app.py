...
# Helper to load teacher credentials
def load_teachers():
    with open(TEACHERS_FILE, "r") as f:
        data = json.load(f)
    return {t["username"]: t["password"] for t in data["teachers"]}

def get_current_teacher(session_id: str = Cookie(None)):
...
@app.get("/whoami")
def whoami(teacher: str = Depends(get_current_teacher)):
    if teacher:
        return {"role": "teacher", "username": teacher}
    return {"role": "student"}
